...
def detect_end_contact(data, sample_rate = 60): 
    ...
    stable_window_start = 0
    stable_window_end   = 1*sample_rate 

   
    baseline_data    = data[stable_window_start:stable_window_end]
    base_line_mean   = baseline_data.mean()
    baseline_std     = np.std(baseline_data)
    threshold        = base_line_mean + 20 * baseline_std # Regra empírica do desvio padrão (Adaptada empiricamente :) )

    end_contact_index = max_height_index

    while(data[end_contact_index]>threshold):
        end_contact_index -= 1

    return end_contact_index, threshold # Retirar thresholds



def find_end_propulsion_oc(bk_file_full_path):

    pos_file_path  = os.path.join(bk_file_full_path,"analyze_BodyKinematics_pos_global.sto" )
    ...
    end_prop_index,threshold = detect_end_contact(tyr_pos_column)

    print("Altura halúx[início de salto]:{height_start:3f} ({time_start:.3f})s \n".format(height_start = tyr_pos_column[end_prop_index],time_start = time_column[end_prop_index]) )
#############################################################################
    #show_column_figure(time_column,tyr_pos_column ,"posição dedão","metros",threshold,time_column[end_prop_index])
#############################################################################
    return end_prop_index
...
def crop_signal(signal, max_height_index, sample_rate=60, time=4):
...
def normalized_mae(fp_signal, oc_signal):
    ...
    # desconsiderando valores NaN e usando a amplitude (pico a vale) do da plataforma de força por ser menos ruidoso.
    ...
    if valid_fp_signal.size == 0 or valid_oc_signal.size == 0:
        raise ValueError("Os sinais não contêm valores válidos para cálculo.")

    # Calcular amplitude (pico a vale) do sinal de referência
